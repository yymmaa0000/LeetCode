    ...
    def maxVacationDays(self, flights: List[List[int]], days: List[List[int]]) -> int:
        ...
        for j in range(k):
            ...
            for i in range(city):
                vacation = days[i][j]
                best = -2**31
                for old in range(city):
                    if old == i or flights[old][i]: best = max(best,dp[old])
                new_dp[i] = best + vacation
            dp = new_dp
            #print(dp)
        return max(dp)
