...
class Solution:
    def cheapestJump(self, A: List[int], B: int) -> List[int]:
        dic = {}
        def dfs(A,B,dic,current):
            #print(current)
            if current in dic: return dic[current]
            if current >= len(A):
                return 2**31
            if A[current] == -1:
                return 2**31
            ...
            result_cnt = 2**31
            for i in range(B):
                temp1 = dfs(A,B,dic,current+i+1)
                if temp1 < result_cnt:
                    result_cnt = temp1
            if result_cnt != 2**31:
                result_cnt += A[current]
            dic[current] = result_cnt
            return result_cnt
       
        temp1= dfs(A,B,dic,0)
        print(dic)
        if temp1 == 2**31: return []
        result = []
        i = 0
        while i < len(A):
            result.append(i+1)
            ...
            next_i = 0
            for j in range(B):
                if i+j+1 >= len(A):continue
                if A[i+j+1] == -1: continue
                if dic[i+j+1] < next_val:
                    next_val = dic[i+j+1]
                    next_i = i+j+1
            i = next_i
